                    ...
                    <div class="card-header">
                        <div class="row">
                            ...
                            <div class="col-lg-6 d-flex justify-content-end">
                                <a href="{{ url('jabatan') }}" class="btn btn-sm btn-primary">Kembali</a>
                            </div>
                        </div>
                    </div>
                    <div class="card-body custom-input">
                        <form id="edit_jabatan" class="row g-3 needs-validation" action="{{ url('jabatan/edit/save') }}/{{ $jabatan->id }}" method="POST">
                            @csrf
                            @method('PUT')
                            <div class="col-md-6">
                                <label class="form-label" for="nama">Nama<span class="txt-danger">*</span></label>
                                <input class="form-control" id="nama" type="text" name="nama" value="{{ $jabatan->nama }}" required>
                            </div>
                            <div class="col-md-6">
                                <label class="form-label" for="gaji">Gaji<span class="txt-danger">*</span></label>
                                <input class="form-control" id="gaji" type="text" name="gaji" value="{{ $jabatan->gaji }}" required>
                            </div>
...
<script>
    $(document).ready(function() {
        $("#submit").on('click', function(e) {
            ...
            let formUrl = $('#edit_jabatan').attr('action');
            let form_data = new FormData($('#edit_jabatan')[0]);
